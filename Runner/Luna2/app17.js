...
async function loadStartPoint() {
  ...
  for (const path of candidates) {
    try {
      systemStatus.textContent =
        "Henter startpunkt: " + path;

      const response = await fetch(
        path + "?build=17&t=" + Date.now(),
        { cache: "no-store" }
      );

      if (!response.ok) {
        throw new Error(
          path + " gav HTTP " + response.status
        );
      }
      ...
      if (
        ...
        throw new Error(
          path + " indeholder ugyldige koordinater"
        );
      }
  ...
  throw lastError ||
    new Error("Startpunkt kunne ikke hentes");
}

async function loadRoute() {
  const response = await fetch(
    "banen.gpx?build=17&t=" + Date.now(),
    { cache: "no-store" }
  );
...
async function initialize() {
  try {
    systemStatus.textContent =
      "Trin 1/3 · Henter startpunkt…";

    await loadStartPoint();

    systemStatus.textContent =
      "Trin 2/3 · Henter banen…";

    await loadRoute();

    systemStatus.textContent =
      "Trin 3/3 · Beregner checkpoints…";
