const STORAGE_KEY="stafet_checkpoints_v23_fix1";
...
function handleAfterFinish(point){
  ...
  const oppositeStraightDistance=distanceBetween(point,checkpoints[2]);
  ...
  // VIGTIG RETTELSE:
  // Retningen må ikke afgøres ud fra, hvilket sving der blot er nærmest.
  // GPS og banens geometri kan ellers vælge forkert lige efter mål.
  // Vi venter i stedet, til et rigtigt checkpoint er bekræftet.

  // Første sving betyder: personen fortsætter en ny normal omgang.
  if(!afterFinishBackSide && firstTurnDistance<=ZONE_RADIUS){
    wrongHits[1]+=1;
  ...
  // Andet sving betyder: personen er gået tilbage efter mål.
  // Det er tilladt og må ikke give fejl.
  if(secondTurnDistance<=ZONE_RADIUS){
    wrongHits[3]+=1;
    if(wrongHits[3]>=REQUIRED_HITS){
      afterFinishBackSide=true;
      msgEl.textContent="Efter mål · tilbage ved ANDET SVING er tilladt";
      renderProgress();
    }
    return;
  }else{
  ...
  // Først hvis personen fortsætter helt videre til modsatte langside,
  // har vedkommende gået for langt baglæns rundt på banen.
  if(afterFinishBackSide && oppositeStraightDistance<=ZONE_RADIUS){
    wrongHits[2]+=1;
  ...
  // Mellem mål og et af svingene afventer appen roligt.
  // Der gives ingen falsk rækkefølgefejl i dette område.
}
