const STORAGE_KEY="stafet_checkpoints_v20";
...
async function loadBundledCheckpoints(){
  try{
    const response=await fetch(
      "checkpoints.json?v=20&t="+Date.now(),
      {cache:"no-store"}
    );
...
function registerStep(){
  ...
  if(step===4){
    ...
    /*
      MÅL er samtidig START på næste omgang.
      Derfor skal næste forventede punkt være
      første sving, ikke START igen.
    */
    step=1;
  }else{
    step+=1;
