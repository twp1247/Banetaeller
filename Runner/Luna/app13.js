...
async function loadSharedStartPoint() {
  ...
  statusText.textContent = "Henter fælles start/mål…";
  statusText.dataset.state = "ready";
  startBtn.disabled = true;

  const startPointUrl =
    "/Banetaeller/startpoint.json?build=13&t=" +
    Date.now();

  try {
    const response = await fetch(startPointUrl, {
      cache: "no-store"
    });
    ...
    if (
      ...
      throw new Error(
        "startpoint.json indeholder ikke et gyldigt punkt"
      );
    }
    ...
    statusText.textContent =
      startPoint.accuracy !== null
        ? "Startpunkt hentet ✅ · GPS ±" +
          Math.round(startPoint.accuracy) +
          " m"
        : "Startpunkt hentet ✅";
    ...
    console.error(
      "Kunne ikke hente fælles start/mål:",
      error
    );
...
function startRunner() {
  ...
  if (!trackReady || !startPoint) {
    statusText.textContent =
      "Vent – fælles start/mål indlæses";
    return;
  }
